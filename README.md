...
-   **`media.js`**: Offers a `MediaContext` for subscribing to screen resize and scale change events, allowing you to create responsive components. It can also automatically update a wrapper element with responsive CSS classes based on the current screen size and scale.
...
-   **`common.js`**: Provides utility functions for unit conversions (`pxToRem`, `remToPx`), and for getting the current screen size (`getSize`) and scale (`getScale`).
...
<img src="resources/gameface_tooltip.gif" width="720" alt="Example of a custom Gameface tooltip.">
...
<img src="resources/gameface_inject.gif" width="720" alt="Example of injecting content into the hangar footer.">
